...
class VarnishAdminSocket
{
    ...
    public function __construct($host = '127.0.0.1', $port = 6082, $v = '2.1')
    {
        ...
        } elseif (3 === $this->version) {
            // @todo sanity check 3.x number
        } elseif (4 === $this->version) {
            // @todo sanity check 4.x number
        } elseif (5 === $this->version) {
            // @todo sanity check 5.x number
        } elseif (6 === $this->version) {
            // @todo sanity check 6.x number
        } else {
            throw new Exception('Only Varnish versions from 2 to 6 are supported');
        }
        $this->ban = in_array($this->version, [3, 4, 5, 6]) ? 'ban' : 'purge';
    }
